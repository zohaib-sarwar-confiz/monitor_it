...
module MonitorIt
  class Rack
    def initialize(app, options = {:subscribe_me => {:options => [], :enabled_environments=> [], :host => "", :port => "", :create_path => "/request/create", :fetch_path => "/" }})
      @app = app
      ...
      if options[:subscribe_me][:host].present? and options[:subscribe_me][:port].present? and options[:subscribe_me][:create_path].present? and options[:subscribe_me][:fetch_path].present?
